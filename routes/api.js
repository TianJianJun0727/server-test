...
/**
 * 获取模板列表（依赖 senderId）
 * GET /api/templates?senderId={senderId}
 */
 ...
 * GET /api/languages?templateId={templateId}
 */
 ...
 * GET /api/template-parameters?templateId={templateId}&language={language}
 */
router.get('/template-parameters', (req, res) => {
  const { templateId, language } = req.query;
...
router.post('/workflow-action', (req, res) => {
  const { sender, selectTemplate, selectLanguage, templateParameters } = req.body;
